    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-6">
            <nav class="flex space-x-4" aria-label="Tabs">
                <button type="button"
                    class="tab-button active bg-blue-100 text-blue-700 px-3 py-2 rounded-md text-sm font-medium"
                    data-tab="news">
                    Noticias ({{ $pendingNews->total() }})
                </button>
                <button type="button" class="tab-button bg-gray-100 text-gray-500 px-3 py-2 rounded-md text-sm font-medium"
                    data-tab="polls">
                        ...
                        <li class="px-6 py-4">
                            <div class="flex items-center justify-between">
                                <div class="flex-1">
                                    ...
                                    <p class="mt-2 text-xs text-gray-500">Por: {{ $news->author->name }} •
                                ...
                                <div class="flex space-x-2">
                                    ...
                                    <form method="POST" action="{{ route('communications.moderation.reject') }}" class="inline">
                                        ...
                                        <input type="hidden" name="content_type" value="news">
                                        <input type="hidden" name="content_id" value="{{ $news->id }}">
                                        <input type="text" name="notes" placeholder="Razón del rechazo"
                                            class="border px-2 py-1 text-sm mr-2" required>
                                        <button type="submit"
                        ...
                        <li class="px-6 py-4">
                            <div class="flex items-center justify-between">
                                <div class="flex-1">
                                    ...
                                    <p class="mt-2 text-xs text-gray-500">Por: {{ $poll->creator->name }} •
                                        {{ $poll->created_at->diffForHumans() }}</p>
                                </div>
                                <div class="flex space-x-2">
                                    <form method="POST" action="{{ route('communications.moderation.approve') }}"
                                        class="inline">
                        ...
                        <li class="px-6 py-4">
                            <div class="flex items-center justify-between">
                                <div class="flex-1">
                                    ...
                                    <p class="mt-2 text-xs text-gray-500">Para: {{ $shoutout->employee->name }} •
                                        {{ $shoutout->created_at->diffForHumans() }}</p>
                                </div>
                                <div class="flex space-x-2">
                                    <form method="POST" action="{{ route('communications.moderation.approve') }}"
                                        class="inline">
